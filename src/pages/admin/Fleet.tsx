...
export default function Fleet() {
  ...
  const openDocsDialog = (vehicle: Vehicle) => {
    setSelectedVehicle(vehicle);
    setDocsDialogOpen(true);
    fetchDocs(vehicle.id);
  };
  ...
  const handleUploadDoc = async () => {
    if (!docFile || !selectedVehicle || !docName.trim()) {
      toast.error("Preencha o nome e selecione um ficheiro");
      return;
    }
    setUploading(true);
    ...
    if (uploadError) {
      toast.error("Erro ao enviar ficheiro: " + uploadError.message);
      setUploading(false);
      return;
    }
    ...
    if (error) {
      toast.error("Erro ao guardar documento: " + error.message);
    } else {
      toast.success("Documento adicionado");
      setDocName("");
      setDocType("other");
      setDocFile(null);
      fetchDocs(selectedVehicle.id);
    }
    setUploading(false);
  };
  ...
  const handleDeleteDoc = async (docId: string) => {
    ...
    if (error) toast.error(error.message);
      ...
      if (selectedVehicle) fetchDocs(selectedVehicle.id);
    }
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <Dialog open={docsDialogOpen} onOpenChange={setDocsDialogOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Documentos — {selectedVehicle?.plate}</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-3 border rounded-lg p-3">
            ...
            <div className="flex gap-2">
              <Button type="button" variant="outline" size="sm" className="flex-1" onClick={() => fileInputRef.current?.click()}>
                ...
                {docFile ? docFile.name : "Selecionar Ficheiro"}
              </Button>
              <Button size="sm" onClick={handleUploadDoc} disabled={uploading || !docFile || !docName.trim()}>
